import React, { useEffect, useState } from 'react';
import { View, Text, StyleSheet, FlatList, TouchableOpacity, Alert } from 'react-native';
import { auth, db } from '../firebaseConfig';
import { collection, query, where, getDocs, doc, getDoc, updateDoc } from 'firebase/firestore';
import styles from './styles/stylesmyconnections';

export default function MyConnectionsScreen() {
  const [familyConnections, setFamilyConnections] = useState<any[]>([]);
  const [healthcareConnections, setHealthcareConnections] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const loadConnections = async () => {
      const currentUserId = auth.currentUser?.uid;
      if (!currentUserId) return;
  
      const connectedUsers: any[] = [];
  
      const queries = [
        query(
          collection(db, 'connectionRequests'),
          where('toUserId', '==', currentUserId),
          where('status', '==', 'accepted')
        ),
        query(
          collection(db, 'connectionRequests'),
          where('fromUserId', '==', currentUserId),
          where('status', '==', 'accepted')
        )
      ];
  
      for (const q of queries) {
        const snapshot = await getDocs(q);
  
        for (const requestDoc of snapshot.docs) {
          const requestData = requestDoc.data();
          const otherUserId =
            requestData.fromUserId === currentUserId
              ? requestData.toUserId
              : requestData.fromUserId;
  
          const userSnap = await getDoc(doc(db, 'users', otherUserId));
          if (userSnap.exists()) {
            const userData = userSnap.data();
            connectedUsers.push({
              id: requestDoc.id,
              name: userData.name || 'N/A',
              lastName: userData.lastName || '',
              email: userData.email || '',
              userType: userData.userType || '',
            });
          }
        }
      }
  
      
      setFamilyConnections(connectedUsers.filter(u => u.userType === 'Family'));
      setHealthcareConnections(connectedUsers.filter(u => u.userType === 'Healthcare'));
      setLoading(false);
    };
  
    loadConnections();
  }, []);
  

  const confirmRemove = (requestId: string) => {
    Alert.alert(
      'Are you sure?',
      'This will remove the connection.',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Remove',
          style: 'destructive',
          onPress: () => handleRemove(requestId),
        },
      ]
    );
  };

  const handleRemove = async (requestId: string) => {
    try {
      await updateDoc(doc(db, 'connectionRequests', requestId), {
        status: 'removed',
      });
      setFamilyConnections(prev => prev.filter(conn => conn.id !== requestId));
      setHealthcareConnections(prev => prev.filter(conn => conn.id !== requestId));
      Alert.alert('Connection removed.');
    } catch (error) {
      console.error('Error removing connection:', error);
      Alert.alert('Error', 'Could not remove connection.');
    }
  };

  const renderConnection = ({ item }: { item: any }) => (
    <View style={styles.card}>
      <Text style={styles.name}>{item.name} {item.lastName}</Text>
      <Text style={styles.email}>{item.email}</Text>
      <Text style={styles.role}>Role: {item.userType}</Text>
      <TouchableOpacity
        onPress={() => confirmRemove(item.id)}
        style={styles.removeButton}
      >
        <Text style={styles.removeButtonText}>Remove</Text>
      </TouchableOpacity>
    </View>
  );

  return (
    <View style={styles.container}>
      <Text style={styles.title}>Your Connections</Text>

      {loading ? (
        <Text style={styles.subtitle}>Loading connections...</Text>
      ) : (
        <>
          {familyConnections.length > 0 && (
            <>
              <Text style={styles.sectionTitle}>Family</Text>
              <FlatList
                data={familyConnections}
                keyExtractor={(item) => item.id}
                renderItem={renderConnection}
              />
            </>
          )}

          {healthcareConnections.length > 0 && (
            <>
              <Text style={styles.sectionTitle}>Healthcare</Text>
              <FlatList
                data={healthcareConnections}
                keyExtractor={(item) => item.id}
                renderItem={renderConnection}
              />
            </>
          )}

          {familyConnections.length === 0 && healthcareConnections.length === 0 && (
            <Text style={styles.subtitle}>You are not connected to anyone</Text>
          )}
        </>
      )}
    </View>
  );
}
